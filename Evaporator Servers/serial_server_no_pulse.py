...
class SerialServer(LabradServer):
    ...
    def enumerate_serial_windows(self):
        ...
        for a in range(1, 48):
            ...
            try:
                ser = Serial(dev_name, dsrdtr = True)
                ser.close()
    ...
    def enumerate_serial_pyserial(self):
        ...
        for d in dev_list:
            dev_path = d[0]
            try:
                ser = Serial(dev_path, dsrdtr = True)
                ser.close()
            except SerialException as e:
                pass
            else:
    ...
    @setting(10, 'Open',
             port=[': Open the first available port',
                   's: Port to open, e.g. COM4'], DTR = 'b',
             returns=['s: Opened port'])
    def open(self, c, port='', DTR = None):
        ...
        if not port:
            ...
            for x in self.SerialPorts:
                if os.path.normcase(x.name) == os.path.normcase(port):
                    try:
                        c['PortObject'] = Serial(x.devicepath, timeout=0, dsrdtr = DTR)
                        return x.name
